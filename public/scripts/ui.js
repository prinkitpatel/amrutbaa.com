...
// ===============================================
// VIDEO MANAGEMENT - MUTE/UNMUTE & AUTOPLAY
// ===============================================

export function initVideoManagement() {
    window.toggleMute = function (container) {
        const video = container.querySelector("video");
        if (!video) return;
        video.muted = !video.muted;
    };

    // Sync Custom Mute UI to Actual Video State
    document.querySelectorAll(".video-container video").forEach((video) => {
        ...
        updateMuteIcons(); // Initial sync
    });

    // Smart Autoplay based on Scroll Visibility
...
// ===============================================
// STORY CAROUSEL - Mobile Snap Logic
// ===============================================

export function initStoryCarousel() {
    const storiesGrid = document.querySelector(".food-stories-grid");
    const storyCards = document.querySelectorAll(".story-card");
    const scrollDots = document.querySelectorAll(".scroll-dot");

    if (!storiesGrid || scrollDots.length === 0) return;
    ...
    storiesGrid.addEventListener(
        ...
        () => {
            ...
            scrollTimeout = setTimeout(() => {
                ...
                const cardWidth = storyCards[0]?.offsetWidth || 0;
                const gap = 20;
...
export function initHeroAnimation() {
    const heroAnimatedText = document.getElementById("heroAnimatedText");
    if (!heroAnimatedText) return;

    const phrases = ["Zero Guilt.", "100% Fresh.", "Shahi Flavour."];
    let phraseIndex = 0;

    setInterval(() => {
        heroAnimatedText.style.transition = "opacity 0.4s ease, transform 0.4s ease";
        heroAnimatedText.style.opacity = "0";
        heroAnimatedText.style.transform = "translateY(-15px)";

        setTimeout(() => {
            phraseIndex = (phraseIndex + 1) % phrases.length;
            heroAnimatedText.textContent = phrases[phraseIndex];
            heroAnimatedText.style.transition = "none";
            heroAnimatedText.style.transform = "translateY(15px)";

            setTimeout(() => {
                heroAnimatedText.style.transition = "opacity 0.4s ease, transform 0.4s ease";
                heroAnimatedText.style.opacity = "1";
                heroAnimatedText.style.transform = "translateY(0)";
            }, 50);
        }, 400);
    }, 3000);
}